...
const EventDetail = ({ events }) => {
  ...
  const handleProceedToBooking = () => {
    if (selectedShowtime) {
      navigate(`/booking-event/${id}/${selectedShowtime}`);
    }
  };
    ...
    <section className="text-gray-600 body-font overflow-hidden">
      <div className="container px-5 py-24 mx-auto">
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          <img alt={event.eventName} className="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" src={event.eventImgUrl} />
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            <h2 className="text-sm title-font text-gray-500 tracking-widest">EVENT</h2>
            <h1 className="text-gray-900 text-3xl title-font font-medium mb-1">{event.eventName}</h1>
            <div className="flex mb-4">
              <span className="text-gray-600">Genre: {event.genre}</span>
            </div>
            <p className="leading-relaxed">{event.eventDetails}</p>
            <p className="leading-relaxed text-gray-600">Date: {formattedEventDate}</p>
